...
function epm_template_update_select_files_config(e) {
	...
	$.getJSON(url, function (data) {
		...
		$.each(data.only_configs, function (key, val) {
			if (val['select'] == "ON") {
				select.append('<option data-icon="fa fa-files-o" value="' + val[id] + '_' + val[label] + '" selected>' + val[label] + '</option>');
			}
			else {
				select.append('<option data-icon="fa fa-files-o" value="' + val[id] + '_' + val[label] + '">' + val[label] + '</option>');
			}
		});
